...
graph TD
    ...
    subgraph Quality_Layer ["质量保障 (Quality Assurance)"]
        ...
        A08 -.-> A04
        A11[11. 音频调试手册 Debug Cookbook] -.-> A04
        A11 -.-> A05
    end
...
*   **🆕 AudioFlinger 深度扩充**: Track 状态机、FastTrack vs NormalTrack、重采样器、Buffer 链路、Dump 实战
*   **🆕 AudioPolicy 深度扩充**: XML 完整解析、路由决策全链路、Engine 架构、设备连接处理、AudioPatch
...
*   **🆕 ASoC 深度扩充**：DAPM 状态机 / Widget 图 / 电源序列 / FE-BE DPCM / DMA 传输 / Device Tree 绑定
*   TinyALSA 原理与使用
...
*   **🆕 Auracast 广播音频**与 Isochronous Channels 传输机制
*   **🆕 Android BT Audio HAL**：AIDL 接口、A2DP 数据通路、LE Audio 集成
*   **🆕 蓝牙音频调试**：btsnoop HCI log 分析、codec 协商诊断

### [11. 音频调试手册 (Audio Debug Cookbook)](./11-Audio-Debug-Cookbook)
